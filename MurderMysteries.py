...
def murder_type(weapon, possible_murder_types):
    if weapon in murder_weapon_list:
...
def has_injuries():
    if selected_injury_location is not None:
        return True
    else:
        return False
...
selected_number_of_witness, selected_is_witness_present = number_of_witnesses()
selected_victim_name = victim_name(victim_name_list)
selected_has_injuries = has_injuries()
